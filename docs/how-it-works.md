...
- Owns `ports: ["3000:3000"]` — the host-facing binding
- Image: `dpivot/proxy:latest` (the built-in Go binary)
...
- `DPIVOT_CONTROL_PORT=9900` — HTTP control API port
...
Client → :3000 → proxy accepts connection
               ...
               router.Next() picks a backend (round-robin across non-draining backends)
                       ↓
               dial backend.Addr (e.g. "web:3000" via DNS on dpivot_mesh)
                       ↓
...
5. GET the new container's IP on dpivot_mesh via docker inspect
6. POST http://localhost:9900/backends {"id":"web-<id>","addr":"<ip>:3000"}
...
10. docker compose up -d --no-deps --scale web=1 web
    → Docker stops the old container (the extra replica)
```
...
- `addr` — the dial address (`host:port`)
...
All backend management during a rollout goes through `http://localhost:9900` (or whichever port is set in `DPIVOT_CONTROL_PORT`). The API is reachable only from within `dpivot_mesh` — not from the Docker host or the public network.

If you set `DPIVOT_API_TOKEN`, the proxy requires a `Authorization: Bearer <token>` header on all backend management endpoints. `/health` is always unauthenticated.
...
2. Attempts `docker compose up -d --no-deps --scale web=1 web` to clean up the extra replica
...
All proxied services and their corresponding proxy containers join `dpivot_mesh`:

```
Docker host
├── :3000 → dpivot-proxy-web (dpivot_mesh)
│                    ↓
│             web container (dpivot_mesh, expose 3000, no host binding)
│
├── :5432 → db container (direct, not proxied — postgres is excluded)
│
└── :9900 → dpivot-proxy-web control API (dpivot_mesh only)
```
